...
try:
    assert os.path.exists(os.path.join(config.data['ds009']['datadir'],subject_id))
except:
...
runinfo.iterables = ('runcode',['1','2'])

taskinfo = pe.Node(interface=niu.IdentityInterface(fields=['taskname']), name="taskinfo")

taskinfo.iterables = ('taskname',['emotionalregulation','stopsignal'])
...
datasource_anat.inputs.base_directory = config.data['ds009']['datadir']
...
datasource_func.inputs.base_directory = config.data['ds009']['datadir']

datasource_func.inputs.template = '%s/func/%s_task-%s_run-0%s_bold.nii.gz'

datasource_func.inputs.template_args = dict(func=[['subject_id','subject_id','taskname','runcode']])
...
try:
    workdir='/Users/poldrack/data_unsynced/fmri-handbook-2e-data/ds009/nipype_workdir'
    assert os.path.exists(workdir)
except:
    workdir=os.path.join(config.data['ds009']['datadir'],'nipype_workdir')
...
datasink.inputs.base_directory = os.path.join(config.data['ds009']['datadir'],'derivatives')
...
datasource_func.inputs.template = 'derivatives/rescaled/_runcode_%s/_subject_id_%s/_taskname_%s/%s_task-%s_run-0%s_bold_mcf_brain_filt_maths.nii.gz'

datasource_func.inputs.template_args = dict(func=[['runcode','subject_id','taskname','subject_id','taskname','runcode']])
...
def get_onsets(subject_id,runnum,taskname):
    from fmrihandbook.utils.config import Config
    from fmrihandbook.utils.BIDSto3col import bids_to_3col
    from nipype.interfaces.base import Bunch
    import numpy
    ...
    condition_columns={}
    condition_columns['stopsignal']=['trial_type_orig']
    condition_columns['emotionalregulation']=['trial_type','image_type']
    parametrics={}
    parametrics['stopsignal']=[]
    parametrics['emotionalregulation']=['response']
    config=Config()

    regressors=[]
    regressor_names=[]
    fdfile=os.path.join(config.data['ds009']['datadir'],'derivatives/mcflirt/par/_runcode_%s/_subject_id_%s/_taskname_%s/fd.txt'%(runnum,subject_id,taskname))
    regressors.append(numpy.loadtxt(fdfile))
    regressor_names.append('fd')

    dvarsfile=os.path.join(config.data['ds009']['datadir'],'derivatives/mcflirt/par/_runcode_%s/_subject_id_%s/_taskname_%s/dvars.txt'%(runnum,subject_id,taskname))
    regressors.append(numpy.loadtxt(dvarsfile))
    regressor_names.append('dvars')

    motfile=os.path.join(config.data['ds009']['datadir'],
                'derivatives/mcflirt/par/_runcode_%s/_subject_id_%s/_taskname_%s/%s_task-%s_run-0%s_bold_mcf.nii.gz.par'%(runnum,subject_id,taskname,subject_id,taskname,runnum))
    ...
    eventsfile=os.path.join(config.data['ds009']['datadir'],'%s/func/%s_task-%s_run-0%s_events.tsv'%(subject_id,subject_id,taskname,runnum))
    assert os.path.exists(eventsfile)
    onsetbase=os.path.join(config.data['ds009']['datadir'],'%s/func/onsets/%s'%(subject_id,taskname))
    ons=bids_to_3col(eventsfile,onsetbase,condition_columns[taskname],parametrics[taskname])
...
datasource_antsreg.inputs.base_directory = config.data['ds009']['datadir']
...
datasource_linearreg.inputs.base_directory = config.data['ds009']['datadir']
